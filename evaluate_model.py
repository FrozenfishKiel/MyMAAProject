...
def main():
    ...
    try:
        ...
        while True:
            ...
            # 因为你把挂机(10)删了，所以当它死锁时，我们强行让它换一张卡！
            if consecutive_blocks >= 3:
                print(f"\n[ACTION MASKING] AI连续 {consecutive_blocks} 次被阻断陷入死锁！强行打乱它的选择...")
                action[0] = (action[0] + 1) % 12  # 强行换下一张卡尝试
                action[1] = env.action_space.sample()[1] # 随机换个位置
                consecutive_blocks = 0

            obs, reward, terminated, truncated, info = env.step(action)

            # 记录连续非法拦截（惩罚现在是 -0.05，所以判断阈值改小）
            if reward < 0: # 只要是负分（被拦截、被拒收）就累计死锁
